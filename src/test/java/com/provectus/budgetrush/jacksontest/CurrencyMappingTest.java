...
@ContextConfiguration(classes = { ObjectMapper.class, InMemoryConfig.class, CurrencyService.class })
@WebAppConfiguration
public class CurrencyMappingTest {
    ...
    public void jsonMappingTest() throws Exception {
        ...
        mapper.writeValue(file, currencyService.getById(1));
        scanner = new Scanner(file);
        log.info(scanner.nextLine());
        scanner.close();

        log.info("Cresting POJO from JSON");
        Currency currency = mapper.readValue(file, Currency.class);
        log.info(currency.toString());

        file.delete();

        assertNotNull(file.toString(), currency);
    }
